...
void WiFiManager::setMinimumSignalQuality(int8_t quality) {
  ...
  if (scan) {
    ...
    if (n == 0) {
      ...
      for (uint8_t i = 0; i < n; i++) {
        ...
        int8_t quality = getRSSIasQuality(WiFi.RSSI(indices[i]));
...
int8_t WiFiManager::getRSSIasQuality(int8_t RSSI) {
  int8_t quality = 0;
